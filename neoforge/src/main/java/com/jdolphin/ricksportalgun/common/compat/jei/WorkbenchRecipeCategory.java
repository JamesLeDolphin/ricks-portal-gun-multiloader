...
public class WorkbenchRecipeCategory implements IRecipeCategory<PortalGunWorkbenchRecipe> {
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, PortalGunWorkbenchRecipe recipe, IFocusGroup focuses) {
        ...
        if (!ingredients.isEmpty()) {
            builder.addSlot(RecipeIngredientRole.INPUT, 11, 14).add(ingredients.getFirst().isEmpty() ? ItemStack.EMPTY : ingredients.getFirst());

            if (size > 1) {
                builder.addSlot(RecipeIngredientRole.INPUT, 63, 14).add(ingredients.get(1).isEmpty() ? ItemStack.EMPTY : ingredients.get(1));
            }
            if (size > 2) {
                builder.addSlot(RecipeIngredientRole.INPUT, 11, 37).add(ingredients.get(2).isEmpty() ? ItemStack.EMPTY : ingredients.get(2));
            }
            if (size > 3) {
                builder.addSlot(RecipeIngredientRole.INPUT, 63, 37).add(ingredients.get(3).isEmpty() ? ItemStack.EMPTY : ingredients.get(3));
            }

            builder.addSlot(RecipeIngredientRole.OUTPUT, 37, 67).add(recipe.getResult());
        }
    }
}